...
| 2026-08-30 | R4-A 审查修复 | 裸实例 getattr 兼容 + except 留痕; failed 诚实透传 (不折算 pending_approval); readonly 假成功修复 (去恒真路径匹配 + 检查 ok + 特异路径优先); fs executor 调用形态修正 (Callable 契约, 此前真实执行从未成功); ACTION_SEMANTICS 移除 ASK 类误导映射; 公开 attach_decision_bridge; run.py 明示 ASK 队列无消费方; AUTO 写文件豁免登记 EVOLUTION 文档 §六; import_rules allowlist 补 ocos.execution 边; engine_bridge 循环导入治愈 (_ensure_engines_registered 幂等兜底) | tests/ 1962 passed + ocos/tests/ 3283 passed = 合并 testpaths 5245 passed 零失败; ocos.db 污染还原 | c19468b |
