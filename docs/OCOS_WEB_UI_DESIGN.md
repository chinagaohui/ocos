...
| 阶段 | 内容 |
|---|---|
| v1 | 单文件聊天页 + converse/summary/approvals API |
| v2（当前，2026-08-30） | **"数字生命"形态升级**：全屏 canvas 粒子神经球生命核（斐波那契球面 220 节点+预计算神经连线，呼吸/思考/回应三态驱动转速与脉动）、三环 HUD 旋转光环、星尘背景、悬浮对话气泡（老消息渐隐）、生命体征面板 + 能量条、待批卡片右下角、核心状态文字（待命中/处理中/回应中） |
| v3 | SSE 流式回复；语音输入/输出（Web Audio 可视化波形接入说话态）；消息经收件箱喂 daemon（行为与对话同流）；trace 时间线视图；记忆/信念浏览页 |
| v4 | Telegram 等外发通道复用同一 API；多生命体并存视图 |
