...
class GrowthEngine:
    ...
    def apply_in_sandbox(self, proposal: GrowthProposal) -> GrowthResult:
        ...
        护栏 ①-⑦ 全部检查通过才能真 apply.
        Step 4: 接入 CircuitBreaker 熔断机制.
        """
        from ocos.growth.shadow_verifier import (
            ShadowVerifier, EvolutionGuard, is_red_line,
        )
        from ocos.governance.risk_registry import on_growth_failure, on_growth_success
        ...
        if not ok:
            ...
            on_growth_failure(reason=result.reason)  # Step 4 hook
            return result
        ...
        try:
            ...
            result.status = "applied"
            on_growth_success()  # Step 4 hook — 重置熔断器
            return result
